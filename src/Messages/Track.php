...
class Track implements MessageInterface
{
    public string $type        = '';
    ...
    {
        if ($this->event === '') {
            throw new FieldException('analytics.Track', 'Event', $this->event);
        }
        if ($this->userId === '' && $this->anonymousId === '') {
            throw new FieldException('analytics.Track', 'UserId', $this->userId);
        }
    }
