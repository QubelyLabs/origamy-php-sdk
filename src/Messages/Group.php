...
class Group implements MessageInterface
{
    public string $type        = '';
    ...
    {
        if ($this->groupId === '') {
            throw new FieldException('analytics.Group', 'GroupId', $this->groupId);
        }
        if ($this->userId === '' && $this->anonymousId === '') {
            throw new FieldException('analytics.Group', 'UserId', $this->userId);
        }
    }
